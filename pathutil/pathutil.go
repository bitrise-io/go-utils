...
func AbsPath(pth string) (string, error) {
	...
	if strings.HasPrefix(pth, "~") {
		...
		if len(pth) == 0 {
			return filepath.Abs(os.ExpandEnv("$HOME"))
		}
		...
		tUser, err := user.Lookup(username)
		if err != nil {
			return "", fmt.Errorf("failed to find home path of user: %s, error: %s", username, err)
		}

		return filepath.Abs(os.ExpandEnv(filepath.Join(tUser.HomeDir, strings.Join(splitPth[1:], "/"))))
	}
